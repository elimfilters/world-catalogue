...
async function syncSheets() {
    try {
        ...
        rows.forEach(row => {
            ...
            if (sku) {
                if (uniqueData.has(sku)) {
                    duplicatesCount++;
                } else {
                    uniqueData.set(sku, {
                        input_code: row.get('Input Code'),
                        sku: sku,
                        description: row.get('Description'),
                        micron_rating: row.get('Micron Rating'),
        ...
        const filterData = Array.from(uniqueData.values());
        console.log('?? Datos detectados: ' + rows.length);
        console.log('?? Duplicados ignorados en el Excel: ' + duplicatesCount);
