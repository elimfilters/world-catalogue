const mongoose = require('mongoose');
const { JWT } = require('google-auth-library');
const { GoogleSpreadsheet } = require('google-spreadsheet');
const Kit = require('../models/kitModel');
require('dotenv').config();

async function syncKits() {
    try {
        console.log('?? Conectando a MongoDB para Kits...');
        await mongoose.connect(process.env.MONGO_URL || process.env.MONGODB_URI);
        
        const clientEmail = "elimfilters-writer@gen-lang-client-0000922456.iam.gserviceaccount.com";
        const spreadsheetId = "1ZYI5c0enkuvWAveu8HMaCUk1cek_VDrX8GtgKW7VP6U";
        const rawKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

        const auth = new JWT({
            email: clientEmail,
            key: rawKey.replace(/\\n/g, '\n'),
            scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'],
        });

        const doc = new GoogleSpreadsheet(spreadsheetId, auth);
        await doc.loadInfo();
        const sheet = doc.sheetsByTitle['MASTER_KITS_V1'];
        const rows = await sheet.getRows();

        const kitEntries = [];
        for (const row of rows) {
            const sku = row.get('kit_sku');
            if (sku) {
                // Procesamos la columna 'filters_included' que parece traer los SKUs separados por comas
                const filtersRaw = row.get('filters_included') || '';
                const filterList = filtersRaw.split(',').map(f => ({
                    filter_sku: f.trim(),
                    quantity: 1
                })).filter(f => f.filter_sku !== '');

                kitEntries.push({
                    kit_sku: sku.trim(),
                    description: row.get('kit_description_en') || '',
                    machine_model: row.get('equipment_applications') || '',
                    filters: filterList
                });
            }
        }

        if (kitEntries.length > 0) {
            console.log('?? Limpiando colección de Kits...');
            await Kit.deleteMany({});
            await Kit.insertMany(kitEntries);
            console.log('?? SINCRO EXITOSA: ' + kitEntries.length + ' kits guardados.');
        } else {
            console.log('?? No se encontraron datos en la columna "kit_sku".');
        }
        
        process.exit(0);
    } catch (error) {
        console.error('? ERROR EN KITS:', error.message);
        process.exit(1);
    }
}
syncKits();
