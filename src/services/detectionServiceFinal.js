// ============================================================================
// DETECTION SERVICE FINAL - v5.2.1
// Fix: Replaced prefixMap.normalize with normalize.code
...
function cleanAppsList(list, duty) {
    ...
    const seen = new Set();
    const cleaned = [];
    // Cero Invención: eliminar genéricos y contradicciones por duty
    const GENERIC_SET = new Set([
        ...LD_ENGINE_DEFAULTS.map(x => String(x.name || '').toUpperCase()),
        ...LD_EQUIPMENT_DEFAULTS.map(x => String(x.name || '').toUpperCase()),
        ...HD_ENGINE_DEFAULTS.map(x => String(x.name || '').toUpperCase()),
        ...HD_EQUIPMENT_DEFAULTS.map(x => String(x.name || '').toUpperCase())
    ]);
    const HD_CONTRA_TOKENS = /(COMMERCIAL TRUCKS|CONSTRUCTION EQUIPMENT|AGRICULTURAL EQUIPMENT|MINING MACHINERY|BUSES AND COACHES|HEAVY MACHINERY|GENERATORS|MARINE EQUIPMENT|INDUSTRIAL EQUIPMENT|FORESTRY EQUIPMENT|RAIL EQUIPMENT|HEAVY DUTY DIESEL ENGINES)/i;
    const LD_CONTRA_TOKENS = /(GASOLINE|HYBRID)/i;
    for (const item of arr) {
        let name = '';
        let years = '';
        ...
        // Filtrar genéricos
        if (GENERIC_SET.has(upper)) continue;
        // Contradicciones Diesel/Gasoline según duty
        if (duty === 'HD' && LD_CONTRA_TOKENS.test(upper)) continue;
        if (duty === 'LD' && HD_CONTRA_TOKENS.test(upper)) continue;
        // Requerir marca detectable o especificidad (cilindrada/años/tokens V6/V8)
        const hasBrand = OEM_MANUFACTURERS.some(b => upper.includes(b));
        const hasSpecificity = /(\b\d\.\dL\b|\b\dL\b|\bV\d\b|INLINE-\d|I\d)/i.test(upper) || /\d{4}/.test(years) || /\d/.test(upper);
        if (!hasBrand && !hasSpecificity) continue;
        const key = `${upper}|${years}`;
        if (!seen.has(key)) {
            seen.add(key);
...
function ensureMinApps(list, duty, kind) {
    ...
    // Política inviolable: no rellenar con genéricos; devolver lista actual limitada
    if (skuPolicyConfig && skuPolicyConfig.enforceInviolable) {
...
async function detectFilter(rawInput, lang = 'en', options = {}) {
    try {
        ...
        if ((!scraperResult || !scraperResult.last4)) {
            try {
                const upHD = normalize.code(query);
        ...
        if (!scraperResult || !scraperResult.last4) {
            const codeUpper2 = normalize.code(query);
            const looksDonaldson = prefixMap.DONALDSON_STRICT_REGEX.test(codeUpper2);
            const looksFram = /^(CA|CF|CH|PH|TG|XG|HM|G|PS)\d/i.test(codeUpper2);
            ...
            if (skuPolicyConfig.allowHdAfRsDonaldsonResolution && duty === 'HD' && /^(AF\d{3,}|RS\d{3,})/i.test(codeUpper2)) {
                try {
                    ...
                    if (pcode) {
                        ...
                        if (donRes && donRes.last4) {
                            ...
                            console.log(`✅ Resuelto HD AF/RS → Donaldson: ${query} → ${pcode}`);
                        }
                    }
                } catch (afrsErr) {
                    console.log(`⚠️  Error resolviendo AF/RS a Donaldson: ${afrsErr.message}`);
                }
            }
            ...
            const up2 = normalize.code(query);
        ...
        if (!homologationOk) {
            const codeUpper3 = normalize.code(query);
            const looksDonaldson = prefixMap.DONALDSON_STRICT_REGEX.test(codeUpper3);
            const looksFram = /^(CA|CF|CH|PH|TG|XG|HM|G|PS)\d/i.test(codeUpper3);
            console.log(`⛔ Homologación no cumplida: duty=${duty}, source=${sourceUp}, family=${familyUp}. looksDonaldson=${looksDonaldson} looksFram=${looksFram}`);
            ...
            // Excepción: LD ECOGARD S-series permite fallback OEM curado
            {
                const up3 = normalize.code(query);
...
const equipFinal = ensureMinApps(equipFmt, duty, 'equipment');
        ...
        try {
            if (/^(EM9|ET9)/.test(String(sku))) {
                ...
                console.log(`✅ Upserted to Google Sheet 'Marinos': ${sku}`);
            } else {
